	...
	for x in tile_offset:
		for y in tile_offset:
			for z in tile_offset:
				...
				grid_instance.translation = Vector3(x * tile_size, y * tile_size, z * tile_size)
...
func _process(delta):
	if is_instance_valid(player):
		for t in tiles:
			...
			if abs(dir.z) > dist:
				offset.z = sign(dir.z) * tile_size * tile_offset

			if abs(dir.x) > dist or abs(dir.y) > dist or abs(dir.z) > dist:
				t.global_translate(offset)
				t.rotate_object_local(Vector3.UP, deg2rad(degrees[randi() % degrees.size()]))
	else:
